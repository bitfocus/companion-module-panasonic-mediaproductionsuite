    ...
    return {
        ...
        trackingControl: {
            ...
            callback: async (action) => {
                const cameraId = Number(action.options.camera_id);
                const options = {};
                if (action.options.pt_speed !== 'unchanged')
                    options.pt_speed = Number(action.options.pt_speed);
                if (action.options.z_speed !== 'unchanged')
                    options.z_speed = Number(action.options.z_speed);
                if (action.options.sensitivity !== 'unchanged')
                    options.sensitivity = Number(action.options.sensitivity);
        ...
        autoFaceSearch: {
            ...
            callback: async (action) => {
                const cameraId = Number(action.options.camera_id);
                const mode = Number(action.options.mode);
        ...
        currentFrameRelative: {
            ...
            callback: async (action) => {
                ...
                const zoomRatio = Number(action.options.auto_zoom_ratio);
                if (targetX !== 0)
                    options.target_x = targetX;
                if (targetY !== 0)
                    options.target_y = targetY;
                if (zoomRatio !== 0)
                    options.auto_zoom_ratio = zoomRatio;
        ...
        getImage: {
            ...
            callback: async (action) => {
                if (!instance.api)
                    return;
                const cameraId = Number(action.options.camera_id);
                const category = String(action.options.category);
        ...
        atCameraControl: {
            ...
            callback: async (action) => {
                const cameraId = Number(action.options.camera_id);
                const control = String(action.options.control);
        ...
        atTracking: {
            ...
            callback: async (action) => {
                const cameraId = Number(action.options.camera_id);
                const process = String(action.options.process);
        ...
        atTrackingControl: {
            ...
            callback: async (action) => {
                const cameraId = Number(action.options.camera_id);
                const enable = String(action.options.enable);
        ...
        vmDsk: {
            ...
            callback: async (action) => {
                const control = Number(action.options.control);
